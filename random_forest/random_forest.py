...
class RandomForestClassifier():
    ...
    def predict_proba(self, X: np.ndarray):
        ...
        for est in self.estimators:
            predict_proba_sample.append(est.predict_proba(X))

        result = np.array(predict_proba_sample).mean(axis=0)
...
class RandomForestRegressor:
    ...
    def predict(self, X: np.ndarray):
        ...
        for est in self.estimators:
            predict_sample.append(est.predict(X))

        result = np.array(predict_sample).mean(axis=0)
